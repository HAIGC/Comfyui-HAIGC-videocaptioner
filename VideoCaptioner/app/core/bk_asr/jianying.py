...
class JianYingASR(BaseASR):
    ...
    def submit(self) -> str:
        ...
        try:
            ...
            logger.error(f"JianYing API returned invalid JSON. Status: {response.status_code}, Content: {response.text[:200]}...")
            raise ValueError(f"剪映接口响应异常 (状态码: {response.status_code})。请尝试切换到 FasterWhisper 本地模型。")
    ...
    def _upload_sign(self):
        ...
        try:
            ...
            logger.error(f"JianYing Upload Sign API returned invalid JSON. Status: {response.status_code}")
            raise ValueError(f"剪映接口上传验证失败 (状态码: {response.status_code})。请尝试切换到 FasterWhisper 本地模型。")
